...
int main(int argc, char * argv[]) {	
	...
	for (k=0;k<MAX_ITERATIONS;k++) {
                ...
                MPI_Win_create(&grid[0],sizeof(grid),sizeof(double),MPI_INFO_NULL,MPI_COMM_WORLD,&win); 

                MPI_Win_fence(0,win);
                if (myrank >0) {
                   disp=local_nx*ny;
                   MPI_Get(&grid[0],ny,MPI_DOUBLE,myrank-1,disp,ny,MPI_DOUBLE,win);
                }
                if (myrank < size-1) {
                   disp=ny;
                   MPI_Get(&grid[(local_nx+1) * ny],ny,MPI_DOUBLE,myrank+1,disp,ny,MPI_DOUBLE,win);
                } 
                MPI_Win_fence(0,win);
                MPI_Win_free(&win);
//		if (myrank > 0) {
